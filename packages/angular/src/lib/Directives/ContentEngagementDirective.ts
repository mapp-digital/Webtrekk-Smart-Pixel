...
export class ContentEngagementDirective implements AfterContentInit {
    @Input('wt-content-engagement') wtContentEngagementElement: WebtrekkContentEngagementProps;
    ...
    ngAfterContentInit() {
        this.pixel.call((pix) => {
            pix.extension.content_engagement.add({
                selector: ((this.wtContentEngagementElement.selector) ? this.wtContentEngagementElement.selector : this.elementRef.nativeElement),
                name: this.wtContentEngagementElement.name,
