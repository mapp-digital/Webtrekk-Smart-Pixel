...
context('Vue2 Content Engagement', () => {
    ...
    it("content engagement leave page", () => {
        ...
        cy.wait(5000).then(() => {
            ...
            expect(linkClick.pu).to.match(/.*apps\/vue2\/$/);
            expect(linkClick.ct).to.match(/.*apps\/vue2\/$/);

            // homepagePI
            expect(homepagePI.eid).to.match(/^\d{19}$/);
            expect(homepagePI.p[1]).to.match(/.*apps\/vue2\/$/);
            expect(homepagePI.pageName).to.match(/.*apps\/vue2\/$/);
            expect(homepagePI.pu).to.match(/.*apps\/vue2\/$/);
        });
