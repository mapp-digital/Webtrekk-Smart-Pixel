...
context("Angular15 complete order process", () => {
    beforeEach(() => {
        cy.smartIntercept();
    ...
    it("login", () => {
        ...
        cy.contains("Your username is: Mapp")
            ...
            .then(() => {
                ...
                expect(pageRequest.p[1]).to.match(/.*apps\/angular15\/login$/);
                expect(pageRequest.pageName).to.match(
                    /.*apps\/angular15\/login$/
                );
                expect(pageRequest.pu).to.match(/.*apps\/angular15\/login$/);
                expect(pageRequest.fns).to.equal("1");
                expect(pageRequest.one).to.equal("1");

                expect(loginEvent.eid).to.match(/^\d{19}$/);
                expect(loginEvent.p[1]).to.match(/.*apps\/angular15\/login$/);
                expect(loginEvent.pageName).to.match(
                    /.*apps\/angular15\/login$/
                );
                expect(loginEvent.pu).to.match(/.*apps\/angular15\/login$/);
                expect(loginEvent.ct).to.equal("Login");
                expect(loginEvent.cd).to.equal("Mapp");

                expect(followPage.eid).to.match(/^\d{19}$/);
                expect(followPage.p[1]).to.match(/.*apps\/angular15\/account$/);
                expect(followPage.pageName).to.match(
                    /.*apps\/angular15\/account$/
                );
                expect(followPage.pu).to.match(/.*apps\/angular15\/account$/);
            });
    });

    it("account", () => {
        const trackData = [];
        cy.visit("/apps/angular15/account");
        cy.wait("@trackRequest").then((inception) => {
            trackData.push(parseTrackrequest(inception));
        });
        cy.contains("Your username is: Mapp")
            ...
            .then(() => {
                ...
                expect(pageRequest.pu).to.match(/.*apps\/angular15\/account$/);
                expect(pageRequest.fns).to.equal("1");
                expect(pageRequest.one).to.equal("1");
            });
    });
    ...
    it("shop list and add", () => {
        ...
        cy.wait("@trackRequest")
            ...
            .then(() => {
                ...
                expect(pageRequest.eid).to.match(/^\d{19}$/);
                expect(pageRequest.p[1]).to.match(/.*apps\/angular15\/shop$/);
                expect(pageRequest.pageName).to.match(
                    /.*apps\/angular15\/shop$/
                );
                expect(pageRequest.pu).to.match(/.*apps\/angular15\/shop$/);
                expect(pageRequest.fns).to.equal("1");
                expect(pageRequest.one).to.equal("1");

                expect(productList.eid).to.match(/^\d{19}$/);
                expect(productList.p[1]).to.match(/.*apps\/angular15\/shop$/);
                expect(productList.pageName).to.match(
                    /.*apps\/angular15\/shop$/
                );
                expect(productList.pu).to.match(/.*apps\/angular15\/shop$/);
                ...
                expect(product1add.p[1]).to.match(/.*apps\/angular15\/shop$/);
                expect(product1add.pageName).to.match(
                    /.*apps\/angular15\/shop$/
                );
                expect(product1add.pu).to.match(/.*apps\/angular15\/shop$/);
                ...
                expect(product2add1.p[1]).to.match(/.*apps\/angular15\/shop$/);
                expect(product2add1.pageName).to.match(
                    /.*apps\/angular15\/shop$/
                );
                expect(product2add1.pu).to.match(/.*apps\/angular15\/shop$/);
                ...
                expect(product2add2.p[1]).to.match(/.*apps\/angular15\/shop$/);
                expect(product2add2.pageName).to.match(
                    /.*apps\/angular15\/shop$/
                );
                expect(product2add2.pu).to.match(/.*apps\/angular15\/shop$/);
    ...
    it("Single product multi add", () => {
        const trackData = [];
        cy.visit("/apps/angular15/shop/6");
        cy.wait("@trackRequest").then((inception) => {
            trackData.push(parseTrackrequest(inception));
        });
        ...
        cy.wait("@trackRequest").then((inception) => {
            ...
            expect(pageView.p[1]).to.match(/.*apps\/angular15\/shop\/6$/);
            expect(pageView.pageName).to.match(/.*apps\/angular15\/shop\/6$/);
            expect(pageView.pu).to.match(/.*apps\/angular15\/shop\/6$/);
            ...
            expect(productAdd.p[1]).to.match(/.*apps\/angular15\/shop\/6$/);
            expect(productAdd.pageName).to.match(/.*apps\/angular15\/shop\/6$/);
            expect(productAdd.pu).to.match(/.*apps\/angular15\/shop\/6$/);
    ...
    it("Add order", () => {
        ...
        cy.wait("@trackRequest").then((inception) => {
            ...
            expect(pageView.eid).to.match(/^\d{19}$/);
            expect(pageView.p[1]).to.match(/.*apps\/angular15\/$/);
            expect(pageView.pageName).to.match(/.*apps\/angular15\/$/);
            expect(pageView.pu).to.match(/.*apps\/angular15\/$/);

            expect(openCartEvent.eid).to.match(/^\d{19}$/);
            expect(openCartEvent.p[1]).to.match(/.*apps\/angular15\/$/);
            expect(openCartEvent.pageName).to.match(/.*apps\/angular15\/$/);
            expect(openCartEvent.pu).to.match(/.*apps\/angular15\/$/);
            expect(openCartEvent.ct).to.equal("Open cart");

            expect(orderRequest.eid).to.match(/^\d{19}$/);
            expect(orderRequest.p[1]).to.equal("phoenix:4001/apps/angular15/thankyou");
            expect(orderRequest.pageName).to.equal("phoenix:4001/apps/angular15/thankyou");
            ...
            expect(closeCartEvent.p[1]).to.match(/.*apps\/angular15\/$/);
            expect(closeCartEvent.pageName).to.match(/.*apps\/angular15\/$/);
            expect(closeCartEvent.pu).to.match(/.*apps\/angular15\/$/);
