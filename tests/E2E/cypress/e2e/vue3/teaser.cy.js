...
context('Vue3 Teaser Tracking', () => {
    beforeEach(() => {
        cy.smartIntercept();
    });

    it('Vue3 teaser views', () => {
        cy.visit('/apps/vue3/teaser');
        const trackData = [];
        ...
        cy.wait('@trackRequest').then(inception => {
            ...
            expect(pageview.p[1]).to.match(/.*apps\/vue3\/teaser$/);
            expect(pageview.pageName).to.match(/.*apps\/vue3\/teaser$/);
            expect(pageview.pu).to.match(/.*apps\/vue3\/teaser$/);
            expect(pageview.fns).to.equal('1');
            expect(pageview.one).to.equal('1');

            // Teaser 1
            expect(teaser1.eid).to.match(/^\d{19}$/);
            expect(teaser1.p[1]).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaser1.pageName).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaser1.pu).to.match(/.*apps\/vue3\/teaser$/);
            ...
            expect(teaser2.p[1]).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaser2.pageName).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaser2.pu).to.match(/.*apps\/vue3\/teaser$/);
    ...
    it('Vue3 teaser click and engagement', () => {
        cy.visit('/apps/vue3/teaser');
        const trackData = [];
        cy.wait('@trackRequest').then(inception => {
            trackData.push(parseTrackrequest(inception));
        ...
        cy.wait('@trackRequest').then(inception => {
            ...
            expect(teaserView.p[1]).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaserView.pageName).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaserView.pu).to.match(/.*apps\/vue3\/teaser$/);
            ...
            expect(clickTracking.p[1]).to.match(/.*apps\/vue3\/teaser$/);
            expect(clickTracking.pageName).to.match(/.*apps\/vue3\/teaser$/);
            expect(clickTracking.pu).to.match(/.*apps\/vue3\/teaser$/);
            expect(clickTracking.ct).to.match(/.*apps\/vue3\/$/);

            // Teaser Click
            expect(teaserClick.eid).to.match(/^\d{19}$/);
            expect(teaserClick.p[1]).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaserClick.pageName).to.match(/.*apps\/vue3\/teaser$/);
            expect(teaserClick.pu).to.match(/.*apps\/vue3\/teaser$/);
            ...
            expect(pageViewTarget.p[1]).to.match(/.*apps\/vue3\/$/);
            expect(pageViewTarget.pageName).to.match(/.*apps\/vue3\/$/);
            expect(pageViewTarget.pu).to.match(/.*apps\/vue3\/$/);
            expect(pageViewTarget.cp1).to.equal('Homepage');

            // Teaser Engagement
            expect(teaserEngagement.eid).to.match(/^\d{19}$/);
            expect(teaserEngagement.p[1]).to.match(/.*apps\/vue3\/$/);
            expect(teaserEngagement.pageName).to.match(/.*apps\/vue3\/$/);
            expect(teaserEngagement.pu).to.match(/.*apps\/vue3\/$/);
