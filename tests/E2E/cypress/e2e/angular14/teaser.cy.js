...
context("Angular14 Teaser Tracking", () => {
    beforeEach(() => {
        cy.smartIntercept();
    });
    ...
    it("Angular teaser views", () => {
        const trackData = [];
        cy.visit("/apps/angular14/teaser");
        ...
        cy.wait("@trackRequest")
            ...
            .then(() => {
                ...
                expect(pageview.p[1]).to.match(/.*apps\/angular14\/teaser$/);
                expect(pageview.pageName).to.match(
                    /.*apps\/angular14\/teaser$/
                );
                expect(pageview.pu).to.match(/.*apps\/angular14\/teaser$/);
                expect(pageview.fns).to.equal("1");
                expect(pageview.one).to.equal("1");

                // Teaser 1
                expect(teaser1.eid).to.match(/^\d{19}$/);
                expect(teaser1.p[1]).to.match(/.*apps\/angular14\/teaser$/);
                expect(teaser1.pageName).to.match(/.*apps\/angular14\/teaser$/);
                expect(teaser1.pu).to.match(/.*apps\/angular14\/teaser$/);
                ...
                expect(teaser2.p[1]).to.match(/.*apps\/angular14\/teaser$/);
                expect(teaser2.pageName).to.match(/.*apps\/angular14\/teaser$/);
                expect(teaser2.pu).to.match(/.*apps\/angular14\/teaser$/);
    ...
    it("Angular14 teaser click and engagement", () => {
        cy.visit("/apps/angular14/teaser");
        const trackData = [];
        cy.wait("@trackRequest").then((inception) => {
            trackData.push(parseTrackrequest(inception));
        ...
        cy.wait("@trackRequest").then((inception) => {
            ...
            expect(pageview.eid).to.match(/^\d{19}$/);
            expect(pageview.p[1]).to.match(/.*apps\/angular14\/teaser$/);
            expect(pageview.pageName).to.match(/.*apps\/angular14\/teaser$/);
            expect(pageview.pu).to.match(/.*apps\/angular14\/teaser$/);
            expect(pageview.fns).to.equal("1");
            expect(pageview.one).to.equal("1");
            ...
            expect(teaserView.p[1]).to.match(/.*apps\/angular14\/teaser$/);
            expect(teaserView.pageName).to.match(/.*apps\/angular14\/teaser$/);
            expect(teaserView.pu).to.match(/.*apps\/angular14\/teaser$/);
            ...
            expect(clickTracking.p[1]).to.match(/.*apps\/angular14\/teaser$/);
            expect(clickTracking.pageName).to.match(
                /.*apps\/angular14\/teaser$/
            );
            expect(clickTracking.pu).to.match(/.*apps\/angular14\/teaser$/);
            expect(clickTracking.ct).to.match(/.*apps\/angular14\/$/);

            // Teaser Click
            expect(teaserClick.eid).to.match(/^\d{19}$/);
            expect(teaserClick.p[1]).to.match(/.*apps\/angular14\/teaser$/);
            expect(teaserClick.pageName).to.match(/.*apps\/angular14\/teaser$/);
            expect(teaserClick.pu).to.match(/.*apps\/angular14\/teaser$/);
            ...
            expect(pageViewTarget.p[1]).to.match(/.*apps\/angular14\/$/);
            expect(pageViewTarget.pageName).to.match(/.*apps\/angular14\/$/);
            expect(pageViewTarget.pu).to.match(/.*apps\/angular14\/$/);

            // Teaser Engagement
            expect(teaserEngagement.eid).to.match(/^\d{19}$/);
            expect(teaserEngagement.p[1]).to.match(/.*apps\/angular14\/$/);
            expect(teaserEngagement.pageName).to.match(/.*apps\/angular14\/$/);
            expect(teaserEngagement.pu).to.match(/.*apps\/angular14\/$/);
