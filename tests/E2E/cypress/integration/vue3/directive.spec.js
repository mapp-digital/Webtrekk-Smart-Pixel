...
context('Vue3 Directives', () => {
    beforeEach(() => {
        cy.smartIntercept();
    });

    it('test directives', () => {
        cy.visit('/apps/vue3/directive');
        cy.wait(1000);
        ...
        cy.wait('@trackRequest').then(inception => {
            ...
            expect(request.p[1]).to.match(/.*apps\/vue3\/directive$/);
            expect(request.pageName).to.match(/.*apps\/vue3\/directive$/);
            expect(request.pu).to.match(/.*apps\/vue3\/directive$/);
